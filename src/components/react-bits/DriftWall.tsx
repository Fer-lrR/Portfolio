...
export const DriftWall: React.FC<DriftWallProps> = ({
  ...
  const maskStyle =
    'radial-gradient(ellipse 85% 85% at 50% 50%, #000 65%, transparent 100%), ' +
  ...
  const renderTile = (item: DriftWallItem, id: string, colIndex: number) => {
    ...
    const inner = (
      <span
        style={{
          ...
          border: isActive ? '2px solid #c25e00' : '1px solid rgba(226, 217, 207, 0.4)',
          opacity: isActive ? 1 : dim,
          transform: isActive ? `translateZ(${lift}px) scale(1.05)` : 'translateZ(0) scale(1)',
          boxShadow: isActive
            ? '0 20px 40px rgba(0, 0, 0, 0.25), 0 0 20px rgba(194, 94, 0, 0.35)'
            : '0 4px 12px rgba(0, 0, 0, 0.08)',
          transition: 'transform 0.35s cubic-bezier(0.22,1,0.36,1), opacity 0.35s ease, border-color 0.35s ease, box-shadow 0.35s ease',
          pointerEvents: 'none'
        }}
      >
        <img
          ...
          style={{
            ...
            objectFit: 'cover',
            userSelect: 'none',
            filter: isActive ? 'grayscale(0) saturate(1.1)' : grayscale ? 'grayscale(1)' : 'saturate(0.95)',
            transition: 'filter 0.35s ease'
          }}
        />

        {/* Dark Vignette / Title Badge */}
        <span
          style={{
            position: 'absolute',
            inset: 0,
            background: 'linear-gradient(to top, rgba(24, 26, 31, 0.9) 0%, rgba(24, 26, 31, 0.2) 60%, transparent 100%)',
            display: 'flex',
    ...
    const commonProps = {
      style: {
        ...
        outline: 'none',
        width: '100%',
        height: `calc(${tileHeight}px + ${gap}px)`,
        ...
        {columnItems.map((col, c) => {
          ...
          return (
            <div
              style={{
                position: 'relative',
                width: `calc(${tileWidth}px + ${gap}px)`,
                transformStyle: 'preserve-3d'
              }}
              key={`col-${c}`}
            >
